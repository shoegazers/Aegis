...
/*
pub async fn take_webcam_photo() -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut urls = Vec::new();

    let index = CameraIndex::Index(0);

    let requested =
        RequestedFormat::new(nokhwa::utils::RequestedFormatType::AbsoluteHighestFrameRate);

    let mut camera = Camera::new(index, requested).unwrap();
    let frame = camera.frame().unwrap();

    let decoded: Img = frame.decode_image::<RgbFormat>().unwrap();
